...
Two threads meet in PBFT, but the book followed more than two. Hewitt's actors and Hoare's processes were rival answers to how isolated things should talk at all, the asynchronous mailbox and the synchronous handshake that every concurrent system still chooses between. Codd and Gray argued over how data should be modeled and kept correct, and Spanner refused to settle it by taking both sides. Lampson and Parnas asked where to draw a boundary and what to hide behind it, the question under every interface and every service split. Set beside replication and trust, these are not separate books. They are the same small set of questions asked of different material: how independent parts coordinate, how a system stays correct when parts of it fail, where state and trust should live, and how to keep the whole thing survivable. That is the claim the preface made, and fifteen papers are the evidence for it.
...
- **Fischer, Lynch, and Paterson, "Impossibility of Distributed Consensus with One Faulty Process" (1985).** The limit PBFT respects rather than beats.
