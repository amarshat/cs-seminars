...
So take sharing away. Armstrong's model, which he names Concurrency Oriented Programming (his spelling, unhyphenated), is built from processes that share nothing at all. A process is a small self-contained world. It has its own memory. No other process can read it or write it. The only thing a process can do to another process is send it a message, and a message is copied, not shared. The receiver gets its own copy and the sender keeps its own. What matters is the guarantee, not the mechanism: because Erlang data is immutable, the runtime is free to share some large values under the hood (big binaries are reference-counted off-heap rather than deep-copied), but no process can ever observe another mutating shared state, because there is no mutable state to share. There is nothing for a fault to travel along.
...
His claim is the reverse, and it is the practical payoff of the whole model. Because processes share nothing, adding more of them cannot corrupt the ones already running. Because you were forced from the start to assume messages can be lost, you are pushed to design for loss rather than discover it in production. That is not the same as getting loss-tolerance for free: you still write the timeouts, the retries, and the idempotency yourself. The model just makes you do it instead of letting you forget. And because a same-machine process and a remote process obey the same source-level rules, the program you wrote for one machine compiles and runs across many. The reasoning, though, does not travel quite so transparently, and it is dishonest to pretend otherwise. Going distributed adds latency you did not have, partition as a failure mode you did not have, and weaker delivery guarantees underneath the uniform interface (local delivery is reliable and ordered; cross-node delivery is neither). Erlang's all-to-all node connectivity also caps a single cluster in the low hundreds of nodes in practice. The source ports with little change. The correctness argument has to be re-examined. Still, the discipline that looked like a tax at design time is what lets the system scale and survive at all, and you paid most of that cost up front instead of during an outage.
