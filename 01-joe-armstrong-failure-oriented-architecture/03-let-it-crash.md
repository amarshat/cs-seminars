...
It works only if restart returns to a good state, and that depends on which fault killed you. A transient fault, the soft-fault majority from chapter 1, is gone on restart: the fresh process meets a different interleaving. A deterministic fault is a trap: a poison message or a corrupt stored row kills the new process the same way it killed the last, and you get a crash loop doing no work. Curing that needs the cause removed (quarantine the message, validate state on load), not another restart. Armstrong's restart-intensity limits, the subject of the next chapter, stop the flapping by giving up and escalating, but escalation contains a fault, it does not cure it. So "let it crash" cures transient faults and merely contains deterministic ones, and treating those two as the same is how you ship a system that crash-loops in production.

It also works only as far as chapter 2's boundary reaches. A crash is safe because the process shares nothing in memory, so it cannot leave a torn data structure for anyone else to read. But that stops at the VM's edge: a process that dies midway through a file write, a socket send, or a stable-storage update can leave torn external state. Armstrong handles that separately (resource cleanup on a linked death, transactional storage), not for free from isolation. The slogan is the visible tip of the isolation model underneath it, and it ends where that model ends.

One last limit, taken up in full in [chapter 5](05-links-and-monitors.md). "Let it crash" assumes fail-stop: a process either works or stops cleanly. Two things break that. A process can fail silently, corrupted but still answering, so nothing crashes and the slogan never fires. And a process can look dead when it is only unreachable, so a supervisor restarts it while the original runs on behind a partition, and now two instances each believe they are the one true worker. Restart-on-suspected-death is safe only when death is real and final; when it might be a partition, you need fencing or a quorum.

## Modern echo

The pattern is now mainstream, usually under different names. Kubernetes does not debug a misbehaving container; a failed liveness probe makes the kubelet on that node restart it in place, which is "let it crash" with the supervisor moved out into the platform (chapter 7 traces exactly which part of the platform). The "cattle, not pets" maxim is the same stance toward processes: do not nurse a sick one back to health, replace it. "Crash-only software," argued by Candea and Fox in the same year as this thesis (and cited in it), reached the same conclusion from the operating-systems side: if the only way to stop is to crash, and the only way to start is to recover, you exercise your recovery path constantly and it actually works. And Kubernetes learned the asterisks too. A pod that keeps dying lands in CrashLoopBackOff, an exponential backoff that bounds the retry rate. Be precise about the match, though: that captures the "give up locally" half of Armstrong's restart limit, not the "escalate to a wider view" half, because a backed-off pod just stays down with no parent to escalate to (chapter 7 returns to this). Still, telecom, operating systems, and cloud orchestration all rediscovered that restarting has to be bounded, and convergence on a limit is stronger evidence that the idea is real than convergence on the slogan alone.
