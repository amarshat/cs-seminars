...
Jim Gray, whose results Armstrong leans on directly, split production software faults into two kinds. Most are transient: a race, a rare timing window, a momentary resource exhaustion, a one-in-a-million interleaving. Gray had observed this for hardware and conjectured the same for software, and it is the conjecture Armstrong quotes: most production faults are soft, and if you reinitialize the state and retry, the operation usually succeeds the second time. For these, redundancy and restart genuinely work. The standby, or a freshly restarted process, meets a slightly different timing and never reproduces the fault. This is not a footnote. It is the reason the entire "let it crash" strategy of chapter 3 works at all.
